...
export class SubToDoListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    var newResource = this.resourceservice;
    this.data.currentGoals.subscribe((res) =>{
       ...
       if(this.subGoalArray == undefined){
         this.subGoalArray = newData.getSessionSubgoalInfo();
    ...
    this.data.currentSubGoalSoloIndex.subscribe((res) =>{
            ...
            if(this.subGoalIndex==undefined){
              this.subGoalIndex = newData.getSubGoalIndexFromSessions();
    ...
    this.data.currentSubGoalSolo.subscribe((res) =>{
        ...
        if(this.currentSubGoal==undefined){
          this.currentSubGoal = newData.getSubGoalFromSession();
      ...
      //  this.goalcount = this.userGoals.length;
        ...
        this.data.currentHomeGoalRes.subscribe((res) =>{
          ...
          if(this.goalIndex==undefined){
            this.goalIndex = newResource.getGoalIndexMarker();
  ...
  onSave(){
      ...
      this.data.passSubGoalToDisplay(this.subGoalArray[this.subGoalIndex]);
      console.log("New goal")
      console.log(this.goalNeeded);
      this.resourceservice.sendNewGoalSubGoal(this.goalNeeded, this.goalIndex, this.emailID);
      this.data.passSubGoalToDisplayAsSession(this.subGoalArray[this.subGoalIndex]); //this way, when I go back to subgoal page and even upon refresh, edits will display automatically
