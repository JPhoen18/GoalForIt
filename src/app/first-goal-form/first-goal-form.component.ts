...
//  });
  ...
  onSubmit() { //first onsubmit called by the formparent
      console.log("Hello in the first goal form")
    this.submitted = true;
 ...
 var userObjArray = [[this.goalModel.goalTitle, this.goalModel.description, this.goalModel.goalCreationDate, this.goalModel.goalDueDate]]
   this.data.passUserObj(userObjArray) // this makes the observable in add resource decision fire before it is supposed to.
   this.resourceservice.storeFirstGoalInfo(userObjArray);
   //That is why the resources are constantly filling up and not deleting. 1/21/21

  }

  async checkForEmptyFields(){
